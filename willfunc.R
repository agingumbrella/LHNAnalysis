...
stacked.plot <- function(data, plot.name="",scale=6) {
	spacing <- 5*sd(as.vector(data), na.rm=T)	
	max.data <- max(as.vector(data))
	min.data <- min(as.vector(data))
	num.total <- ncol(data)
	for (i in 1:num.total) {
		plot(1:nrow(data), data[,i] - spacing*i, type='l', ylim=c(-num.total*spacing,0), xaxt='n', yaxt='n', xlab="", ylab="",frame=T, col='red')
		text(5,(spacing/2)-spacing*i, gcamp.odors[i],1,cex=0.5)
		par(new=T)
...
heatmap(max.dfof[axons,],col=jet.colors(20),scale="row",symm=F)
